...
- `RAG_PDF_OCR_ENABLED`: (Optional) Enable the OCR fallback for text-less PDFs (scanned pages / CAD exports). When a PDF's extracted text layer is below `RAG_PDF_OCR_MIN_CHARS`, each page is rasterized with `pypdfium2` and OCR'd with `rapidocr-onnxruntime` so the document becomes searchable instead of silently ingesting as zero chunks. Default value is "False" (opt-in; existing behavior unchanged).
- `RAG_PDF_OCR_MIN_CHARS`: (Optional) Minimum number of non-whitespace characters a PDF must yield to be treated as having a usable text layer; below this the OCR fallback runs. Default value is "16".
