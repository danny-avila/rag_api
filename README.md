...
- `RAG_PDF_OCR_ENABLED`: (Optional) Enable the OCR fallback for text-less PDFs (scanned pages / CAD exports). The fallback is page-aware: only pages whose extracted text layer is below `RAG_PDF_OCR_MIN_CHARS` are rasterized with `pypdfium2` and OCR'd with `rapidocr-onnxruntime` (native text pages in mixed PDFs are kept as-is), so the document becomes searchable instead of silently ingesting as zero chunks. Default value is "False" (opt-in; existing behavior unchanged).
- `RAG_PDF_OCR_MIN_CHARS`: (Optional) Minimum number of non-whitespace characters a page must yield to be treated as having a usable text layer; pages below this are OCR'd. Default value is "16".
- `RAG_PDF_OCR_DPI`: (Optional) Resolution used to rasterize PDF pages before OCR. Higher values improve accuracy on small text at the cost of speed/memory. Default value is "200".
- `RAG_PDF_OCR_MAX_PAGES`: (Optional) Maximum number of pages OCR'd per document, as a guard against runaway cost on very large files. Default value is "50".
- `RAG_PDF_OCR_MIN_CONFIDENCE`: (Optional) Drop OCR text blocks whose recognition confidence is below this value, so low-quality recognitions never reach the vector store. Range `0.0`–`1.0`; `0.0` keeps everything (default), while a value around `0.5` filters noise on difficult scans/schematics. Default value is "0.0".
