...
class NVIDIAEmbeddings(BaseModel, Embeddings):
    ...
    def _check_port_available(self, url: str, timeout: float = 0.5) -> bool:
        """
        Quick check if the service port is even listening.
        Returns False immediately if nothing is listening on the port.
        ...
        try:
            ...
            # Create a socket and try to connect with very short timeout
            ...
            if result != 0:
                logger.warning(f"NVIDIA service not responding on {host}:{port}")
                return False
            return True
        except Exception as e:
            logger.warning(f"Socket check failed for {url}: {e}")
            return False
    ...
    def _embed_batch_with_retry(self, texts: List[str]) -> List[List[float]]:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                # Fast port check first - fail immediately if nothing is listening
                if attempt == 0 and not self._check_port_available(url, timeout=0.5):
                    raise TimeoutError(f"NVIDIA service not available on {url} (port not listening)")
                ...
                response = requests.post(
                    ...
                    headers=headers,
                    json=request_data,
                    timeout=(2.0, self.timeout)  # (connection_timeout, read_timeout)
                )
                ...
                # For fast failover with backup systems, don't retry timeouts extensively
                # This allows backup providers to take over quickly
                if attempt < self.max_retries - 1 and self.max_retries > 1:
                    time.sleep(0.5)  # Shorter delay before retry
                    continue
                else:
                    raise TimeoutError(f"NVIDIA API timeout after {attempt + 1} attempts") from e
